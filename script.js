...
function syncBodyScrollLock() {
  ...
  const isAddCartOpen = Boolean(addCartModal && !addCartModal.classList.contains("is-hidden"));
  const isSizeChartOpen = Boolean(sizeChartModal && !sizeChartModal.classList.contains("is-hidden"));
  document.body.classList.toggle("is-scroll-locked", isAddCartOpen || isSizeChartOpen);
}
...
function renderCheckoutDeliveryDetails(deliverySummary) {
  ...
  const rows = [
    { label: "Итого за все услуги СДЭК", value: formatCdekPrice(priceValue) },
